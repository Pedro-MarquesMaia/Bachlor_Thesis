...
class Universe():
        ...
        for n,part in enumerate(self.particles):
            ...
            for p in self.particles:
                if p.number != part.number:
                    lower = np.sqrt(((self.to_float(p.position,self.scale) - self.to_float(part.position,self.scale)).length())**2 + self.e**2)
    ...
    """def calc_angular(self):
        c_o_m_p = vector(0,0,0)
        c_o_m_v = vector(0,0,0)
        M = 0
        for part in self.particles:
            M += part.m
            c_o_m_p += part.position*part.m
            c_o_m_v += part.velocity*part.m
        c_o_m_p = c_o_m_p*(1/M)
        c_o_m_v = c_o_m_v*(1/M)
        com_ang = Math.crossproduct(c_o_m_p,c_o_m_v*M)
        rel_com_ang = vector(0,0,0)
        for p in self.particles:
            rel_position = p.position - c_o_m_p
            rel_velocity = p.velocity - c_o_m_v
            rel_com_ang += Math.crossproduct(rel_position,p.m*rel_velocity)
        total_angular = com_ang + rel_com_ang
        return total_angular.length()"""
    def calc_angular(self):
        ang = vector(0,0,0)
        for part in self.particles:
            ang += Math.crossproduct(part.position,part.velocity*part.m)
            #print(part.velocity)
            #print(part.position)
        #print("Total ang")
        #print(ang)
    ...
    def Force(self,m1,m2,p1, p2):
        G = int((6.67*10**(-11))*m1*m2)
        distance = p1 - p2
        Force = distance*(1/(distance.length()+self.e))
        factor = (G) / ((distance.length()+self.e)**2)
        Force = Force * factor
        return Force
    ...
    def remove_particle(self):
        ...
        particle_number = np.random.randint(0,amount-1)
        ...
        for particle in self.particles:
            ...
            for p in self.particles:
                if p.number == particle.number:
                    ...
                    F += self.to_int(self.Force(particle.m, p.m,self.to_float(particle.position,self.scale),self.to_float(p.position,self.scale))*t*(1/particle.m),self.scale)
            V = particle.velocity + F
            particle.velocity = V
        for particle in self.particles:
            #print(particle.number)
            #print(particle.position)
            #print(particle.velocity*int(t/2))
            #print(particle.position + particle.velocity*int(t/2))
            R = particle.position + particle.velocity*int(t/2)
            particle.position = R
            #print("Cross " + str(particle.number) + " : ")
            #print(Math.crossproduct(particle.position,forces[particle.number]))
...
def create_universe(scale):
    cluster1 = create_cluster(20,"red",2*(10**27),-5,200,0,scale)
    cluster2 = create_cluster(20,"blue",2*(10**27),5,300,20,scale)
